...
|------|------|------|----------------|------|
| CO₂ | MH-Z19B | UART | `mhz19` | ✅ 구현완료 |
| 미세먼지 | PMS 시리즈 | UART | `pmsx003` | ✅ 구현완료 |
| 온습도 | SHT20/AHT20 | I2C | `sht3xd` | ✅ 구현완료 |
| TVOC/AQI | ENS160 | I2C | `ens160` | ✅ 구현완료 |
...
## 🎯 ESPHome vs Arduino 비교

| 특징 | ESPHome | Arduino |
|------|---------|---------|
| **개발 난이도** | ⭐⭐ YAML 설정 | ⭐⭐⭐⭐ C++ 프로그래밍 |
| **Home Assistant 연동** | ✅ 자동 | 🔄 수동 구현 필요 |
| **OTA 업데이트** | ✅ 기본 제공 | 🔄 별도 구현 필요 |
| **웹 인터페이스** | ✅ 자동 생성 | 🔄 별도 구현 필요 |
| **센서 지원** | ✅ 광범위 | ⭐⭐⭐ 라이브러리 의존 |
| **안정성** | ✅ 높음 | ⭐⭐⭐ 코드 품질에 의존 |
| **커스터마이징** | ⭐⭐⭐ 제한적 | ✅ 완전 자유 |
...
ESPHome을 사용함으로써:
- **개발 시간 대폭 단축** (YAML 설정만으로 완성)
- **Home Assistant 완벽 연동** (자동 발견, 실시간 데이터)
- **원격 관리 및 업데이트** (OTA, 웹 인터페이스)
- **안정적인 운영** (검증된 플랫폼, 자동 재연결)

단순한 센서 모니터링을 넘어서 **완전한 스마트홈 공기질 관리 시스템**을 구축할 수 있습니다! 🏠✨